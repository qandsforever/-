...
namespace 网络抓包分析
{
    ...
    {
        public static object dicLock = new object();
        /// <summary>
        /// 获取TCP连接对象
        /// </summary>
        ...
        public static Dictionary<string, netProcessInfo> dicNetToProcess = new Dictionary<string, netProcessInfo>();
        /// <summary>
        ///获取udp连接对象
        /// </summary>
        /// <returns></returns>
        ...
        public static void updateNetProcessInfo()
        {
            ...
            {
                string str = row.LocalAddress.ToString() + "-" + row.RemoteAddress.ToString() + ":" + row.RemotePort.ToString();
                if (!dicNetToProcess.ContainsKey(str))
                {
                    netProcessInfo npi = ProcessAPI.GetProcessInfo(row.owningPid);

                    lock (dicLock)
                    {
                        dicNetToProcess.Add(str, npi);
                    }

                }
            }

            foreach (UdpRow row in udpRows)
            {
                string str = row.LocalPort.ToString();
                if (!dicNetToProcess.ContainsKey(str))
                {
                    netProcessInfo npi = ProcessAPI.GetProcessInfo(row.owningPid);
